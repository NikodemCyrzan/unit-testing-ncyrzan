...
describe("QuantityValidator", () => {
	it("throws error if threshold is negative", () => {
		expect(new QuantityValidator(0, 5)).toThrow("Threshold can't be negative");
	});

	it("throws error if packageSize is less than one", () => {
		expect(new QuantityValidator(5, 0)).toThrow("PackageSize can't be less than one");
	});
	...
	it("is not valid when quantity <= 0", () => {
		...
		expect(result.isValid).toBe(false);
		expect(result.error).toBe("Quantity can't be less or equal 0");
	});
});
